...
public class SeqResultWinScreen extends BaseScreen {
	...
	public SeqResultWinScreen() {
		...
		// Bg pattern:
		bg = new BackgroundActor(Assets.trialResultWin.bg_pattern);
		stage.addActor(bg);

		// Scroll:
		scroll = new SimpleActor(Assets.trialResultWin.scroll);
		scroll.setPosition(Constants.WIDTH / 2 - scroll.getWidth() / 2, Constants.HEIGHT / 2 - scroll.getHeight() / 2);
		...
		// Drawings:
		drawings.setPosition(Constants.WIDTH / 2 - drawings.getWidth() / 2,
				Constants.HEIGHT / 2 - drawings.getHeight() / 2);
		...
		if (characters[0] != null) {
			characters[0].setPosition(50 * R, 0);
		}
		if (characters[1] != null) {
			characters[1].setPosition(Constants.WIDTH - 450 * R, 0);
		}
		...
		if (stamps[0] != null) {
			stamps[0].setPosition(Constants.WIDTH / 2 - (1064 / 4 + 390 / 2) * R, 120 * R);
		}
		if (stamps[1] != null) {
			stamps[1].setPosition(Constants.WIDTH / 2 + (1064 / 4 - 390 / 2) * R, 120 * R);
		}
		...
		if (growingScores[0] != null) {
			growingScores[0].setPosition(Constants.WIDTH / 2 - (1064 / 4) * R, 400 * R);
		}
		if (growingScores[1] != null) {
			growingScores[1].setPosition(Constants.WIDTH / 2 + (1064 / 4) * R, 400 * R);
		}

		// Controller:
		stage.addActor(controller);

		// Continue si alguno de los dos ha fallado && 2P en modo historia &&
		// dif. normal:
		if (GameManager.getDifficultyLevel() == EDificulty.MEDIUM
				&& (GameManager.getGameMode() == EGameMode.STORY_2P_ALT
			...
			for (int i = 0; i < GameManager.players.length; i++) {
				if (!GameManager.players[i].getLastTrialSucces()) {
					// Si al que ha fallado lo quedan continues, muestra el
					// continue:
	...
	public void show() {
		...
		if (GameManager.getDifficultyLevel() == EDificulty.MEDIUM
			...
			for (int i = 0; i < GameManager.players.length; i++) {
				if (!GameManager.players[i].getLastTrialSucces()) {
					GameManager.players[i].setActionFocus(continueActor.optionsTable);
					// Con esto se combina la entrada "normal" del inputManager
					// y los los listener:
	...
	private SimpleActor getCharacter(ECharacter selectedChar, boolean success) {
		...
		if (selectedChar == ECharacter.SHO) {
			if (success) {
				reg = Assets.trialResultWin.ch_sho;
			} else {
				reg = Assets.trialResultWin.ch_sho_lose;
			}
		} else if (selectedChar == ECharacter.RYOKO) {
			if (success) {
				reg = Assets.trialResultWin.ch_ryoko;
			} else {
				reg = Assets.trialResultWin.ch_ryoko_lose;
			}
		}
	...
	public SimpleActor getStamp(int score) {
		...
		if (score > Constants.SCORE_GRAND_MASTER) {
			reg = Assets.trialResultWin.stamp_ranking04;
		} else if (score > Constants.SCORE_NINJA_MASTER) {
			reg = Assets.trialResultWin.stamp_ranking03;
		} else if (score > Constants.SCORE_NINJA) {
			reg = Assets.trialResultWin.stamp_ranking02;
		} else {
			reg = Assets.trialResultWin.stamp_ranking01;
		}
			...
			if (!end) {
				if (growingScores[0].isFinished() && growingScores[1].isFinished()) {
					if (row < scoreItems.length) {
						...
						// Si hay un continue entonces esperamos al fin de la
						// cuenta o a que se seleccione una opcion;
